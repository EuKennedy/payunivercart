  ...
  return (
    <main className="grid min-h-screen place-items-center px-6 py-12">
      <div className="w-full max-w-3xl space-y-8">
        <header className="space-y-2 border-[var(--color-border)] border-b pb-6">
          <p className="text-[var(--color-accent-500)] text-xs uppercase tracking-[0.2em]">
            internal · super-admin
          </p>
          <h1 className="font-semibold text-3xl tracking-tight">payunivercart ops</h1>
        ...
        <section className="grid gap-4 sm:grid-cols-3">
          {[
            ...
            >
              <p className="text-[var(--color-fg-subtle)] text-xs uppercase tracking-wider">
                {m.label}
              </p>
              <p className="mt-2 font-semibold text-3xl">{m.value}</p>
            </div>
          ))}
        </section>

        <section className="space-y-3">
          <h2 className="text-[var(--color-fg-subtle)] text-sm uppercase tracking-wider">Áreas</h2>
          <ul className="grid gap-2 sm:grid-cols-2">
            {[
              ...
              <li
                key={href}
                className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-1)] px-4 py-3 text-[var(--color-fg-muted)] text-sm"
              >
                <code className="mr-2 text-[var(--color-fg-subtle)] text-xs">{href}</code>
                {label}
              </li>
            ))}
          </ul>
